...
class IntegrationSetting(TimeStampedModel):
    ...
    ``secret`` holds the API key / password **encrypted at rest** (Fernet via core.crypto)
    and is never returned to the client — only a ``secret_set`` boolean is exposed. The
    matching provider adapter reads these when it is the one selected in ``LMS_ADAPTERS``;
    the console/dev stubs ignore them.
    """
    ...
    # Ciphertext (Fernet) — wider than 255 to hold the encrypted form of a long key.
    ...
    def decrypt_secret(self) -> str:
        """The plaintext secret (for the provider adapter); "" if unset/unreadable."""
