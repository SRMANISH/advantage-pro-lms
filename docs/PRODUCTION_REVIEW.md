...
| **P-09** | Low | 71 plain `APIView`s returned hand-built dicts, so their OpenAPI entries had no request or response body at all — and the noise forced `drf_spectacular.W001/W002` into `SILENCED_SYSTEM_CHECKS`, muffling the `security.W*` signal `check --deploy` exists to protect | ✅ **Closed** — all 71 typed via `@extend_schema` against shared response serializers in `core/schema.py`; schema now generates with **0 errors and 0 warnings** (was 318 errors / 178 warnings) and the silencing has been removed entirely |
